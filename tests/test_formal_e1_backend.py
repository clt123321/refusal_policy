...
class FakeHooks:
    ...
    def run_repair(self, context, arm, parent): return {"valid": True, "arm": arm, "artifact_id": arm, "repair_stage": "r1", "training_recipe_sha256": "same"}
...
def test_full_fake_flow_and_pass_verdict():
    ...
    assert verdict["scientific_evidence"] is False
    assert verdict["scientific_evidence_declared"] is True


def test_construct_rejects_different_cp_training_recipes():
    class DifferentRecipes(FakeHooks):
        def run_repair(self, context, arm, parent):
            result = super().run_repair(context, arm, parent)
            result["training_recipe_sha256"] = arm
            return result

    backend = FormalE1Backend(DifferentRecipes())
    ctx = context()
    pilot = backend.pilot(ctx)
    ctx.state["stages"]["pilot"] = {"result": pilot}
    result = backend.construct(ctx)
    assert result["valid"] is False
    assert result["same_training_recipe"] is False
