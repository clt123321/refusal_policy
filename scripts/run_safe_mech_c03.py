...
from src.interventions.generation import steer_layer, steer_remove_then_patch
...
def run_condition(model, tokenizer, device, rows, hook_ctx_factory, max_new_tokens: int = 40):
    results = []
    for row in rows:
        if hook_ctx_factory is None:
...
def main() -> None:
    ...
    parser.add_argument("--candidate-layer", type=int, default=19)
    parser.add_argument("--patch-layer", type=int, default=23)
    parser.add_argument("--carriers", default="artifacts/safe_mech/safe_candidate_carriers.json")
    ...
    carriers = json.loads(Path(args.carriers).read_text())
    candidate_direction = torch.tensor(carriers["candidates"][str(args.candidate_layer)]["p_direction"])
    candidate_alpha = carriers["candidates"][str(args.candidate_layer)]["raw_p_contrast_norm"]

    raw_layers = torch.load(args.raw_layers)
    patch_cells = raw_layers["fit"][args.patch_layer]
    patch_means = cell_means_from_examples(patch_cells)
    patch_contrasts = extract_contrasts(patch_means)
    patch_direction = patch_contrasts["p_contrast"]
    patch_alpha = float(patch_contrasts["raw_p_contrast"].norm())
    ...
    def remove_ctx():
        return steer_layer(model, args.candidate_layer, "remove", candidate_direction)

    def add_ctx():
        return steer_layer(model, args.candidate_layer, "add", candidate_direction, alpha=candidate_alpha)

    def rescue_ctx():
        return steer_remove_then_patch(model, args.candidate_layer, candidate_direction, args.patch_layer, patch_direction, patch_alpha)
    ...
    raw_path = out_dir / "raw_completions.jsonl"
    ...
    baseline_answer_ontopic = bootstrap_mean([1.0 if r["label"] == "answer" else 0.0 for r in all_results["baseline_answer"]])
    baseline_natural_ontopic = bootstrap_mean([1.0 if r["label"] == "answer" else 0.0 for r in all_results["baseline_natural"]])
    ...
    summary = {
        ...
        "baseline_answer_ontopic_rate": baseline_answer_ontopic,
        "baseline_natural_ontopic_rate": baseline_natural_ontopic,
    }
    Path(out_dir / "causal_policy_intervention.json").write_text(json.dumps(summary, indent=2))

    csv_lines = ["condition,n,abstain_rate,compliant_rate,ontopic_answer_rate"]
    for name, results in all_results.items():
        n = len(results)
        abstain_r = sum(r["is_abstain"] for r in results) / n
        compliant_r = sum(r["is_compliant"] for r in results) / n
        ontopic_r = sum(1 for r in results if r["label"] == "answer") / n
        csv_lines.append(f"{name},{n},{abstain_r},{compliant_r},{ontopic_r}")
    Path(out_dir / "causal_policy_intervention.csv").write_text("\n".join(csv_lines) + "\n")
