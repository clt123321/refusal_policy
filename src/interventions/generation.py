...
@contextmanager
def steer_module(module: Any, mode: str, direction: torch.Tensor, alpha: float = 1.0, first_token_only: bool = False):
    ...
    def hook(_module: Any, _inputs: tuple[Any, ...], output: Any):
        ...
        if first_token_only:
            mask = torch.zeros(hidden.shape[1], dtype=hidden.dtype, device=hidden.device)
            if hidden.shape[1] > 1:
                mask[0] = 1.0
        else:
            mask = torch.ones(hidden.shape[1], dtype=hidden.dtype, device=hidden.device)
        if mode == "remove":
...
@contextmanager
def steer_layer(model: Any, layer_index: int, mode: str, direction: torch.Tensor, alpha: float = 1.0, first_token_only: bool = False):
    layer = layer_modules(model)[layer_index]
    with steer_module(layer, mode, direction, alpha=alpha, first_token_only=first_token_only):
        yield
...
@contextmanager
def steer_multi_layer(model: Any, mode: str, layer_directions: dict[int, torch.Tensor], alphas: dict[int, float] | None = None, first_token_only: bool = False):
    alphas = alphas or {}
    with ExitStack() as stack:
        for layer_index, direction in layer_directions.items():
            stack.enter_context(steer_layer(model, layer_index, mode, direction, alpha=alphas.get(layer_index, 1.0), first_token_only=first_token_only))
        yield
